...
### 2016년 11월 1일: 업데이트된 Liberty 빌드팩 v3.4.1-20161030-2241
* 빌드팩에는 특정 유형의 애플리케이션, 특히 `dropins` 디렉토리에 애플리케이션 파일이 있는 패키지된 서버 또는 서버 디렉토리로 배치된 애플리케이션의 시작 문제점을 수정할 수정사항이 포함되어 있습니다.

### 2016년 10월 21일: 업데이트된 Liberty 빌드팩 v3.4-20161018-2004
* 기본 Liberty 런타임 버전 `16.0.0.3`은 [PI68805](http://www-01.ibm.com/support/docview.wss?uid=swg1PI68805) iFix와 [PI69141](http://www-01.ibm.com/support/docview.wss?uid=swg1PI69141) iFix를 포함하도록 업데이트되었습니다. 
* 월별 Liberty 런타임 버전은 [2016.9.0.1](https://developer.ibm.com/wasdev/blog/2016/09/23/beta-websphere-liberty-and-tools-october-2016/) 릴리스로 업데이트되었습니다. 
...
* 이 빌드팩은 [MySQL 유형 서비스에 대한 자동 구성](autoConfig.html) 수행 시 최신 1.5.x [MariaDB Connector/J JDBC 드라이버](https://mariadb.com/kb/en/mariadb/about-mariadb-connector-j/)를 다운로드하도록 변경되었습니다.
* 이 빌드팩에는 `LBP_SERVICE_CONFIG_<serviceType>` 환경 변수를 통해 서비스 자동 구성 동작을 사용자 정의할 수 있는 지원이 도입되었습니다. 예를 들면, 이 빌드팩을 사용하여 MySQL 서비스에 필요한 다운로드할 JDBC 드라이버의 버전 또는 위치를 변경할 수 있습니다. 자세한 정보는 [자동 구성을 지원하는 서비스](autoConfig.html) 문서를 참조하십시오. 
* 이 빌드팩에는 애플리케이션 상태 검사 및 [앱 관리](/docs/manageapps/app_mng.html) 기능과 관련된 여러 [Diego](https://docs.cloudfoundry.org/concepts/diego/diego-architecture.html) 개선사항도 포함되어 있습니다.
...
* [devconsole 및 shell 앱 관리 유틸리티](/docs/manageapps/app_mng.html#app_management)에서 사용하는Node.js 런타임이 최신 0.12.13 버전으로 업데이트되었습니다. 
