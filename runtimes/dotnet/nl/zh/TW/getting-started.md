...
* 從 [preview4 下載頁面 ![外部鏈結圖示](../../icons/launch-glyph.svg "外部鏈結圖示")](https://github.com/dotnet/core/blob/master/release-notes/download-archives/preview4-download.md) 指示，安裝 .NET Core 1.0.0-preview4-004233 SDK。
* 從 [dot.net 網站 ![外部鏈結圖示](../../icons/launch-glyph.svg "外部鏈結圖示")](https://www.microsoft.com/net/download/core#/runtime)，安裝最新「.NET Core 運行環境」
...
2. 依序按一下「連線」及「連接新服務」。
2. 在「資料與分析」區段中，選取 `ClearDB MySQL Database`，然後建立服務。
...
4. 使用	`dotnet run` 指令，從 `get-started-aspnet-core/src/GetStartedDotnet` 目錄中重新啟動您的應用程式。
  ...
  重新整理位於下列網址的瀏覽器視圖：http://localhost:5000/ 任何您輸入至應用程式的名稱，現在都會新增至資料庫。
