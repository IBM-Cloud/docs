...
- `gcmIcon`：指定要針對通知而顯示之圖示的名稱。請確定圖示已與用戶端應用程式包裝在一起。
...
- `gcmLightsLedArgb`：LED 的顏色。硬體將達成其最佳近似值。
- `gcmLightsLedOnMs`：LED 在閃爍時要開啟的毫秒數。硬體將達成其最佳近似值。
- `gcmLightsLedOffMs`：LED 在閃爍時要關閉的毫秒數。硬體將達成其最佳近似值。
...
- `apnsTitleLocKey`：現行本地化之 Localizable.strings 檔案中標題字串的機碼。可使用 %@ 及 %n$@ 指定元來格式化機碼字串，以接受 `titleLocArgs` 陣列中所指定的變數。
- `apnsLocKey`：現行本地化之 Localizable.strings 檔案中 alert-message 字串的機碼（由使用者的語言喜好設定所設定）。可使用 %@ 及 %n$@ 指定元來格式化機碼字串，以接受 locArgs 陣列中所指定的變數。
