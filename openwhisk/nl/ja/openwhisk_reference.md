...
さらに、アクションがアトミックに実行される保証はありません。2 つのアクションが並行して実行され、その副次作用がインターリーブする可能性があります。副次作用について、{{site.data.keyword.openwhisk_short}} での特定の並行整合性モデルはありません。並行性の副次作用は実装に依存します。
...
アクションは実行中です。デフォルトは `whisk.getAuthKey()` です。 
