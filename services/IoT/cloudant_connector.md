...
4. Select your {{site.data.keyword.cloudant}} configuration options:
  ...
  c. Choose a database name. The database name will be `Iotp_<orgID>_<choice>_<bucket_name>` where `<orgID>` is your organization ID, `<choice>` is your choice of database name, and `<bucket_name>` is a string which defines whether the database uses a daily, weekly, or monthly bucket interval. The `<bucket_name>` selection controls the database name in the following way:
