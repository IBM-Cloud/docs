...
`temp_virt`</li>
    <li>계산 - 올바른 함수를 정의하기 위해 하나 이상의 컴포넌트를 추가합니다. 계산을 빌드하기 위해 특성, 숫자 값 및 수학 연산자(예: +, -, \*, /, (, ) 및 AVG)를 사용할 수 있습니다. AVG 컴포넌트는 지정된 기간 동안 평균 특성 값을 계산하는 데 사용합니다. </br> **중요:** AVG 컴포넌트는 Edge Analytics Agent가 설치된 게이트웨이에 연결된 디바이스에서 선택한 특성의 데이터가 입력되는 경우에만 특성 데이터 점을 리턴합니다. 자세한 정보는 [Edge Analytics Agent 설치](gateways/dashboard.html#edge)를 참조하십시오.</li>
