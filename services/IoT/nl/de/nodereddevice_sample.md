...
Senden Sie mithilfe des Gerätesimulators MQTT-Gerätenachrichten an {{site.data.keyword.iot_short_notm}}. Der Gerätesimulator hat das Senden von Daten für einen Frachtcontainer an einen MQTT-Broker wie beispielsweise {{site.data.keyword.iot_short_notm}} simuliert.
    ...
    3. Suchen die im Servicekatalog den Abschnitt mit den Boilerplates und klicken Sie auf die Option für **Node-RED Starter Community BETA**.**Tipp:** Klicken Sie [hier](https://console.ng.bluemix.net/catalog/starters/node-red-starter/), um direkt zur Seite mit Node-RED Starter zu wechseln.
