...
| 환경 변수        | 정의    |
| ----------------------------|---------------|
| `IBM_CLOUD_DEVOPS_CREDS`    | `credentials` 명령을 사용하여 Jenkins에 정의하는 Bluemix 신임 정보입니다. 예를 들어 `IBM_CLOUD_DEVOPS_CREDS = credentials('BM_CRED')`입니다. 이 명령에서 변수를 설정하면 사용자 이름과 비밀번호에 대해 두 개의 추가 환경 변수 `IBM_CLOUD_DEVOPS_CREDS_USR` 및 `IBM_CLOUD_DEVOPS_CREDS_PSW`가 자동으로 설정됩니다.   |
...
이러한 환경 변수와 신임 정보는 IBM Cloud DevOps 플러그인에서 DevOps Insight와 상호작용하기 위해 사용합니다. 이 예제에서 이는 선언적 파이프라인 형식으로 설정됩니다.  
...
실행해야 하는 경우 항상 파이프라인 정의에 이 단계를 추가합니다. 예를 들어, 테스트를 실행한 후에 테스트 결과를 업로드할 수 있습니다. 그리고 업로드된 이후 게이트에서 해당 결과를 평가할 수 있습니다.  
...
이 단계에서는 하나의 매개변수가 필요합니다. 선택적 매개변수 하나도 승인할 수 있습니다. 
...
이 단계는 2개의 매개변수를 필요로 하며, 선택적 매개변수를 취할 수도 있습니다.  
...
도구 체인과 통합하면 엔드-투-엔드 추적성 및 배치 맵핑이 제공됩니다. 통합 지시사항을 따른 다음  배치 단계 후에 `cf icd --create-connection $IBM_CLOUD_DEVOPS_WEBHOOK_URL $CF_APP_NAME` 명령을 추가하십시오. 이 명령은 Bluemix에서 실행 중인 앱에 Jenkins 통합을 연결합니다. 

이 예제는 전체 배치 단계를 표시합니다. 마지막 명령은 `cf icd --create-connection`입니다.  
...
Jenkins 통합 문서에서 설명된 대로 CloudFoundry CLI 및 CloudFoundry ICD 플러그인이 Jenkins 서버에 설치되어야 합니다. 연결하려면 서버에서 Bluemix에 로그인해야 합니다.

## 선언적 파이프라인의 예제

이 예제는 선언적 Jenkinsfile로 정의된 전체 파이프라인을 표시합니다.  
