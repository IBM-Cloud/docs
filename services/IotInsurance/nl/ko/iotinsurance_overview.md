...
{{site.data.keyword.iotinsurance_short}} 컴포넌트는 이 섹션에 설명된 것과 같이 함께 작동합니다. 이 조직은 아키텍처 다이어그램에도 표시됩니다. {{site.data.keyword.iotinsurance_short}} 대시보드에는 {{site.data.keyword.iot_short_notm}} 및 {{site.data.keyword.cloudantfull}} 데이터베이스에 저장된 데이터가 표시됩니다. Wink 클라우드를 통해 연결된 사용자의 스마트 디바이스는 Transformer로 데이터를 전송하며 여기에서 데이터를 처리한 다음 {{site.data.keyword.iot_short_notm}}에 발송합니다. 이 데이터는 실드 엔진에서 처리하며 실드 기준이 충족되는 경우에는 API를 통해 조치 엔진으로 발송됩니다. 조치 엔진은 {{site.data.keyword.mobilepushfull}}를 사용하여 사용자의 모바일 애플리케이션으로 알림을 발송합니다. 사용자 또한 모바일 애플리케이션을 사용하여 경보나 오퍼링에 응답할 수 있습니다. 이 응답은 {{site.data.keyword.amafull}} 서비스에서 처리하여 API를 통해 {{site.data.keyword.iot_short_notm}}에 리턴한 다음 {{site.data.keyword.iotinsurance_short}} 대시보드로 전달됩니다. 
...
## Transformer
{: #transformer}
Transformer는 클라우드 서버 API에서 새 정보를 요청하여 {{site.data.keyword.iotinsurance_short}}의 데이터와 일치하도록 변환합니다. 그런 다음 데이터는 나머지 {{site.data.keyword.iotinsurance_short}} 구현에서 사용할 수 있도록 공개됩니다. 사용자는 Transformer 컴포넌트에 센서 클라우드 데이터에 액세스하고 기록된 데이터를 처리할 수 있는 권한을 부여해야 합니다. 모바일 스타터 앱을 사용하여 권한을 부여합니다. 현재 지원되는 클라우드 공급업체는 Wink뿐입니다. 
