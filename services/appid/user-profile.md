...
An anonymous user with an identity that is provided by an identity provider can become an identified user. The flow for moving from an anonymous user to a known user is outlined in the following steps:
