 ...
 * [Setting up iOS SDK](getting-started-ios.html)
...
Initialize the SDK by passing the applicationGUID, and applicationRoute parameters. A common, though not mandatory, place to put the initialization code is in the `application:didFinishLaunchingWithOptions` method of your application delegate
...
1. Initialize the Client SDK. Replace the applicationRoute and applicationGUID with values that you obtained from **Mobile Options**.
...
The IMFAuthenticationDelegate sample is designed to work with the Custom Identity Provider sample. You can download the sample from [this Github repository](https://github.com/ibm-bluemix-mobile-services/bms-mca-custom-identity-provider-sample).
...
After you create a custom IMFAuthenticationDelegate, it with `IMFClient`. Call the following code in your application before you send any requests to your protected resources. Use realmName that you specified in the {{site.data.keyword.amashort}} dashboard.
...
{: #custom-ios-testing}
After you intialize the Client SDK and register a custom `IMFAuthenticationDelegate`, you can start making requests to your mobile backend.
