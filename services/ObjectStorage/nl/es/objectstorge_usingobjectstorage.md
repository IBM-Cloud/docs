...
En el ejemplo, ```container_two``` se ha configurado para contener las versiones más antiguas de los objetos almacenados en ```container_one```. Por lo tanto, ```container_one``` tendrá la versión más actualizada de los objetos, y ```container_two``` tendrá las versiones más antiguas de los objetos. Asegúrese de que exista ```container_two``` para que el control de versiones funcione.

Con la configuración del control de versiones, al cargar un objeto a ```container_one```, si hay una versión existente del objeto, la versión existente se mueve a ```container_two``` a medida que se cree la nueva versión en ```container_one```. Si suprime un objeto desde ```container_one```, la versión anterior del objeto se mueve desde ```container_two``` a ```container_one```.

Los objetos del ```container_two``` se nombrarán automáticamente con el siguiente formato: ```<Longitud><Nombre_objeto>/<Indicación de fecha y hora>```

```Longitud``` hace referencia a la longitud del nombre del objeto; hay un número hexadecimal de 3 caracteres del teclado numeral. ```Nombre_objeto``` es el nombre del objeto. ```Indicación de fecha y hora``` es la indicación de fecha y hora de cuándo se cargó originalmente esta versión concreta del objeto.
...
* [path] Vía de acceso completa del objeto expresada como ```/v1/<autorización_cuenta>/<nombre_contenedor>/<nombre_objeto>```. Para obtener más información, consulte el [URL de {{site.data.keyword.objectstorageshort}}](#access-points). 
