...
每一個區段都會平行上傳到名為 ```<container_name>_segments``` 的個別容器。上傳所有區段之後，Swift 會建立資訊清單檔，讓這些區段可以從原始檔名為 ```<file_name>``` 的原始容器 ```<container_name>``` 中下載為單一檔案。

例如，下列指令會從名為 ```test_container``` 的容器中上傳名為 ```large_file``` 的檔案，區段大小為 ```1073741824```。
...
在範例中，```container_two``` 已設定來包含 ```container_one``` 中所儲存物件的舊版本。因此，```container_one``` 將具有最近版本的物件，而 ```container_two``` 將具有物件的舊版本。請確定 ```container_two``` 存在，版本化才能運作。

設定版本化之後，當您上傳物件至 ```container_one``` 時，如果有該物件的現有版本，當新版本在 ```container_one``` 中建立時，現有版本會移至 ```container_two```。如果您從 ```container_one``` 刪除物件，舊版物件會從 ```container_two``` 移回 ```container_one```。

```container_two``` 中的物件將使用下列格式自動命名：```<Length><Object_name>/<Timestamp>```

```Length``` 是指物件的名稱長度；這是 3 個字元並以零填補的十六進位數。
```Object_name``` 是物件的名稱。
```Timestamp``` 是物件的這個特定版本原始上傳時的時間戳記。
...
* 若要設定將在現在起的一小時後刪除物件，請使用下列指令：
  ...
  完成之後，```swift stat container object``` 指令將以新紀元時間顯示 ```X-Delete-At``` 標頭與適當的有效期限。
...
執行 Swift ```stat``` 指令來驗證 ```Temp-URL-Key``` 已順利設定。
...
URL 包含五個部分。```<API version>``` 是第 1 版。您可以從 {{site.data.keyword.objectstorageshort}} 使用者介面中找到 {{site.data.keyword.objectstorageshort}} 的 ```<project ID>```、```<container namespace>``` 及 ```<object namespace>```。如需 ```<access point>```，請參閱下表： 
...
**附註：**從 Keystone 獲得的 ```X-Subject-Token``` 適用於各個儲存空間地區。 
...
**警告：**如果您取消佈建 IBM {{site.data.keyword.objectstorageshort}} for {{site.data.keyword.Bluemix_notm}} 服務實例，則會刪除雲端專案及 Swift 帳戶。已取消佈建實例中的所有容器及物件會從 Swift 中刪除，而且無法還原。
