...
|`GET /v1/{ジオコードまたはロケーション ID}/forecast/daily/{format}.json`   |指定したフォーマットに応じて、地理的位置に対する、3 日、5 日、7 日、10 日間の毎日の天気予報を返します。返される日数は、`3day`、`5day`、`7day`、または `10day` としてフォーマットで指定します。`geocode/{緯度}/{経度}` または `location/{ロケーション ID}` を指定できます。毎日の予報にはそれぞれ、昼間の予報、夜間の予報、および 24 時間の予報を含めることができます。これらの区分は、JSON 応答内の別個のオブジェクトです。毎日の予報の昼間予報データは、現地時間の午後 3 時以降は使用できなくなります。現地時間の午後 3 時に、日中の予報がアプリケーションに表示されないようにしなければなりません。|
|`GET /v1/{ジオコードまたはロケーション ID}/forecast/intraday/{format}.json`|指定したフォーマットに応じて、地理的位置に対する、3 日、5 日、7 日、10 日間における、6 時間の期間の毎日の天気予報を返します。返される日数は、`3day`、`5day`、`7day`、または `10day` としてフォーマットで指定します。`geocode/{緯度}/{経度}` または `location/{ロケーション ID}` を指定できます。毎日の予報にはそれぞれ、午前、午後、夜間、翌日午前の予報を含めることができます。これらの区分は、JSON 応答内の別個のオブジェクトです。|
