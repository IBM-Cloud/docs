...
## Demostración de {{site.data.keyword.weather_short}}
{: #insights_weather_demo}

Puede obtener información sobre cómo recuperar datos meteorológicos con el servicio
{{site.data.keyword.weather_short}} probando la
[app de demostración {{site.data.keyword.weather_short}}](http://weather-company-data-demo.{APPDomain}){: new_window}.
La aplicación se abre en el navegador y le solicita si desea compartir la ubicación actual con
la aplicación. 

En la aplicación de demostración, puede ver las condiciones actuales observadas para la meteorología en la
ubicación. 
...
También puede ver la previsión por hora para las próximas 48 horas y la previsión diaria para los próximos 10 días.
La previsión diaria muestra la parte del día y la parte de la noche.
Si pulsa sobre cada una de estas áreas de la aplicación de demostración, puede
ver los resultados de la llamada de API en formato JSON, que incluye los metadatos que se han utilizado
para recuperar los datos. 

En la aplicación de demostración, pulse **Deploy to Bluemix** para crear una versión clonada de la aplicación o [clone la aplicación directamente desde GitHub](https://github.com/IBM-Bluemix/weather-company-data-demo).

## Obtención de una previsión por hora
{: #getting_twenty_four_hour_forecast}

Puede emitir una operación `GET` en su aplicación para recuperar los datos de la previsión de 48 horas.
...
Por ejemplo, puede utilizar la siguiente solicitud `GET` para recuperar la previsión de 48 horas
para las coordenadas de latitud y longitud para Ottawa, ON, Canadá:
...
```
También puede emitir una operación `GET` en su aplicación para recuperar los datos de la previsión de 48 horas
para un código postal.

**Nota**: Puede proporcionar un código postal solo para los códigos de idioma soportados Estados Unidos (US),
Reino Unido (GB), Francia (FR), Alemania (DE) o Italia (IT).
...
El cuerpo de la respuesta incluye metadatos y una matriz de previsiones por hora para
cada una de las siguientes 48 horas. Por ejemplo,
...
## Obtención de una previsión diaria
...
Puede emitir una operación `GET` en la aplicación para recuperar los titulares de alertas meteorológicas actuales.
Por ejemplo, puede utilizar la solicitud `GET` siguiente para recuperar los titulares de alertas meteorológicas para San Bernardino, CA, EE.UU.:

```
GET https://<username>:<password>@twcservice.mybluemix.net:443/api/weather/v1/geocode/34.12/-117.30/alerts.json?language=en-US
```

El cuerpo de la respuesta incluye metadatos y los titulares de alertas meteorológicas actuales.
Por ejemplo,
los metadatos pueden contener los siguientes datos:
...
Los datos de los titulares de alertas contienen información general y una clave para recuperar los
detalles de alertas meteorológicas. Por ejemplo, los datos de titulares de alertas pueden contener los datos siguientes:
...
La clave del cuerpo de la respuesta de los titulares de alerta se puede utilizar para encontrar detalles de alertas meteorológicas.
Por ejemplo, puede utilizar la solicitud `GET` siguiente para recuperar los detalles de alertas meteorológicas
que se corresponden a los titulares de alertas meteorológicas para San Bernadino, CA, EE.UU. en el ejemplo
anterior:
...
El cuerpo de respuesta proporciona los metadatos e información siguientes:
...
## Obtención de datos de almanaque
{: #getting_almanac_data}

Puede emitir una operación `GET` en la aplicación para recuperar datos de almanaque diarios o mensuales.
Por ejemplo, puede utilizar la siguientes solicitud `GET` para recuperar la información de almanaque diaria para Atlanta, GA, EE.UU.:
...
## Utilización de servicios de ubicación
{: #using_location_services}

### Solicitud por nombre de ciudad
Para solicitar la información de ubicación de una ciudad, debe proporcionar al menos
un parámetro de consulta, como por ejemplo un nombre de ciudad, y un `locationType` de ciudad. También puede
proporcionar el `countryCode` y `adminDistrictCode` para reducir el número de ubicaciones exclusivas
que se pueden recuperar.

Por ejemplo, la solicitud siguiente recupera la información de ubicación para Atlanta, Georgia, EE.UU.
...
El cuerpo de la respuesta contiene la información y los metadatos de la ubicación.
```
{
  "location": [
    {
...
### Solicitud por geocódigo
Para solicitar un conjunto de ubicaciones para un geocódigo, debe proporcionar un par de latitud y longitud válido.

Por ejemplo, la solicitud siguiente recupera la información de ubicación para las geo-coordenadas de Atlanta, Georgia, EE.UU.
...
### Solicitud por código postal
Para solicitar un conjunto de ubicaciones por código postal, debe proporcionar una clave postal, que es un
compuesto de un &lt;código postal&gt;:&lt;código de país&gt;.

Por ejemplo, la solicitud siguiente recupera la información de ubicación para un código postal de Atlanta, Georgia, EE.UU.
