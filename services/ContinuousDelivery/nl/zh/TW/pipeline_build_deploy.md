...
###輸入修訂
當您手動執行階段時，或者它是在階段完成之前執行，則執行中階段會選取其輸入修訂。輸入修訂通常是建置號碼。若要選取輸入修訂，則階段會遵循這些條件：

* 如果選取特定修訂，即使用它。
* 如果未指定特定修訂，請在找到使用相同輸入的階段之前搜尋先前的階段。請尋找並使用該輸入的最後成功執行修訂。
* 如果未指定特定修訂，而且沒有其他階段使用所指定的來源作為輸入，則請使用輸入的最新修訂。
...
您也可以從「階段歷程」頁面執行、取消或配置階段。按一下**執行**以執行階段，或按一下**配置**以配置階段。在階段執行時，按一下執行號碼，然後按一下**取消**，即可予以取消。
