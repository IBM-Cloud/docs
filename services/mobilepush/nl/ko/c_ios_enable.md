...
iOS 애플리케이션이 푸시 알림을 수신하고 사용자 디바이스에 전송하도록 설정할 수 있습니다. 
...
```
2. 터미널에 다음 명령을 입력하여 CocoaPods를 초기화하십시오. Xcode 프로젝트가 있는 디렉토리에서 명령을 실행하십시오. `pod init` 명령에서 파일 제목을 작성합니다.  
```
$ pod init
```
3. 생성된 Podfile에 필요한 SDK 종속 항목을 추가하십시오. 사용자 환경 설정에 따라 다음 Podfile 중 하나를 복사하십시오. 
...
Bluemix에서 작성한 푸시 알림 서비스에 지정된 고유 AppGUID 키를 지정합니다. 
...
디바이스에 애플리케이션을 설치한 후 원격 알림을 수신하려면 애플리케이션(앱)을 APNs에 등록해야 합니다. APNs에서 생성한 디바이스 토큰을 앱에서 수신한 후에는 {{site.data.keyword.mobilepushshort}} 서비스에 이를 되돌려 보내야 합니다. 

iOS 애플리케이션 및 디바이스를 등록하려면 다음을 수행하십시오. 

1. 백엔드 애플리케이션 작성
2. 토큰을 {{site.data.keyword.mobilepushshort}}에 전달
...
3. 디바이스가 알림을 수신했는지 확인하십시오. 다음 이미지는 iOS 디바이스의 포그라운드와 백그라운드에서 {{site.data.keyword.mobilepushshort}}을 처리하는 경보 상자를 표시합니다. 
