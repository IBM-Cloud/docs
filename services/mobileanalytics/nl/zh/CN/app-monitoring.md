...
您可以直观地显示分析存储库中收集的分析数据。这一可视化功能是一种检查特定用例数据的有用方式。除了您报告的定制数据之外，还可以使用由“运行分析”收集的数据来创建图表。
{: #shortdesc}

了解有关应用程序崩溃监视和故障诊断的更多信息。

### 为客户机日志创建定制图表
{: #custom-charts-client-logs}

您可以为包含日志信息的客户机日志创建定制图表，该日志信息是通过平台的记录器 API 发送的。该日志信息还包含设备的上下文信息，其中包括环境、应用程序名称和应用程序版本。
...
2. 在 {{site.data.keyword.mobileanalytics_short}} 控制台中，单击**仪表板**页面上的**定制图表**选项卡。您可以基于发送到服务器的分析消息来创建图表。
3. 单击**创建图表**，以创建新的定制图表。
4. 提供以下值：
  * 图表标题：应用程序和日志级别
  * 事件类型：客户机日志
...
每个定制图表中的数据都可导出为 JSON、XML 或 CSV 格式。
...
您可以在 MobileFirst Analytics Console 中的警报定义内设置阈值，以更好地监视活动。
{: #shortdesc}

您可以配置阈值，当超过阈值时，触发器便会触发警报以向 MobileFirst Analytics Console 监视器通知此情况。已触发的警报可以显示在控制台中，或者可以由定制 webhook 进行处理。此功能能够主动检测客户机日志错误、服务器日志错误、网络等待时间延长以及认证失败。响应式阈值和警报使您不必筛查数据，或者设置大范围的阈值。
...
1. 在 MobileFirst Analytics Console 中，单击铃图标以转至**警报日志**页面。
2. 转至**警报管理**页面，然后单击**创建警报**。
3. 提供以下值：
	* 警报名称：客户机日志的警报
	* 消息：错误消息警报
	* 查询频率：5 分钟
		...
		* 属性：日志级别
			...
			* 阈值
				* 阈值类型：应用程序实例的总计
					...
					注：如果您选择“应用程序的平均值”选项，那么会按设备数来计算客户机日志平均值。例如，如果您有两个设备，其中一个设备发送六个客户机日志，而另一个设备发送三个客户机日志，那么平均值为 4.5 个客户机日志。
				* 运算符：大于或等于 3
...
1. 在 MobileFirst Analytics Console 中，单击**警报**图标。此操作会打开“警报日志”页面。
2. 单击**警报管理**选项卡，然后单击**创建警报**。
3. 提供以下值：
	* 警报名称：应用程序崩溃的警报
	* 消息：应用程序崩溃警报
	* 查询频率：应用程序崩溃
...
您可以在 MobileFirst Analytics Console 中查看应用程序崩溃的相关信息，以更好地监视应用程序和进行故障诊断。
{: #shortdesc}
...
您可以在 IBM MobileFirst™ Analytics Console 的**仪表板**部分中快速查看应用程序崩溃的相关信息。
...
您可以在 IBM MobileFirst Analytics Console 的**应用程序**部分中查看**崩溃**页面，以更好地管理应用程序。
