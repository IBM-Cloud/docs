...
上次更新时间：2016 年 7 月 27 日
{: .last-updated}

使用 Facebook 在 Web 应用程序上认证用户。添加 {{site.data.keyword.amashort}} 安全功能。 
...
针对每一个 {{site.data.keyword.amashort}} 服务实例会自动创建 `VCAP_SERVICES` 环境变量，该环境变量包含授权流程所需的属性。
它包含 JSON 对象，通过单击应用程序<!--the left-side navigator of-->中的**环境变量**，可以查看该
对象。
