# Habilitación de la autenticación de Facebook en apps de iOS
{: #facebook-auth-ios}
...
1. Pulse **Opciones móviles** y anote los valores de *applicationRoute* y *applicationGUID*. Necesitará estos valores cuando inicialice el SDK. 
...
1. Guarde `Podfile` y ejecute el mandato `pod install` desde la línea de mandatos. CocoaPods instala las dependencias. Se mostrarán el progreso y los componentes que se han añadido.  **Importante**: a partir de este momento, tendrá que abrir el proyecto utilizando un archivo `xcworkspace` generado por CocoaPods. El nombre suele ser `{nombre-proyecto}.xcworkspace`.  
...
1. Abra la página principal del panel de control de {{site.data.keyword.Bluemix_notm}} y haga clic en la app. Pulse **Opciones móviles**. Se muestran los valores de *Ruta de aplicación* y *GUID de aplicación*. 
...
3. Inicialice el SDK del cliente. Sustituya applicationRoute y applicationGUID con los valores obtenidos de **Opciones móviles** en el panel de control de {{site.data.keyword.Bluemix_notm}}.
