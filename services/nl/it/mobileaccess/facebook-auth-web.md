...
1. Prendi nota di **Application ID** e **App Secret**. Avrai bisogno di questi valori quando configuri il tuo progetto web per l'autenticazione Facebook nel dashboard {{site.data.keyword.amashort}}.
1. Aggiungi la piattaforma del sito web se non esiste.
1. Aggiungi o apri **Facebook Login** dall'elenco dei prodotti.
1. Immetti l'endpoint di callback del server di autorizzazione nella casella **Valid OAuth redirect URIs**. Trova questo URI di reindirizzamento dell'autorizzazione nei seguenti passi di configurazione del dashboard {{site.data.keyword.amashort}}.
...
1. Fai clic sul tile dell'applicazione pertinente. Viene caricata l'applicazione.
2. Fai clic sul tile per il servizio {{site.data.keyword.amashort}}.
1. Fai clic sul pulsante **Configura** nel pannello **Facebook**.
2. Prendi nota del valore nella casella di testo **Mobile Client Access Redirect URI for Facebook Developer Console**. Questo è il valore che dovrai aggiungere alla casella **Valid OAuth redirect URIs** in **Facebook Login** del portale Facebook Developers nel seguente passo 6.
1. Immetti **Application ID** e **App Secret** di Facebook.
2. Immetti l'URI di reindirizzamento in **Your Web Application Redirect URIs**. Questo valore è per l'URI di reindirizzamento a cui accedere dopo che viene completato il processo di autorizzazione e viene determinato dallo sviluppatore.
3. Fai clic su **Save**.
...
Il passo successivo è quello di ottenere i token di accesso e di identità utilizzando il codice concesso ricevuto precedentemente:
 ...
 1.  Richiama token `tokenEndpoint`, `clientId` e `secret` dalle credenziali del servizio archiviate nella variabile di ambiente `VCAP_SERVICES`. 
    ...
    **Nota:** nel caso tu abbia utilizzato Mobile Client Access prima dell'aggiunta del supporto web potresti non avere l'endpoint token nelle credenziali del servizio. In questo caso utilizza invece i seguenti url in base alla tua regione Bluemix: 
...
In modo che le richieste proteggano le risorse aggiungi un'intestazione dell'autorizzazione alle richieste con la seguente struttura: 
...
* Il `idToken` è facoltativo. Nel caso in cui non hai fornito il token di identità, è possibile accedere alla risorsa protetta ma non si riceveranno le informazioni sull'utente autorizzato. 
