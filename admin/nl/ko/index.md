...
로컬 또는 데디케이티드 환경에 대한 알림을 보고 환경의 상태를 모니터할 수 있습니다. 다양한 알림 유형 및 각각의 알림 유형이 게시되는 위치에 대한 정보는 다음 표를 참조하십시오. 
...
리소스 사용량 정보를 보려면 **관리 &gt; 사용량**을 클릭하십시오.

리소스 모니터링 섹션에서 다음 정보를 볼 수
있습니다.

- 사용된 메모리의 양(GB 단위) 및 디스크 공간의 양(GB 단위)과 같은 리소스 사용 정보를 볼 수
...
데디케이티드 또는 로컬 환경에 대한 계정의 매월 사용량을 볼 수 있습니다. 이 데이터를 사용하여 사용량을 기반으로 특정 조직에 부과할 비용을 식별할 수 있습니다.
...
다음 표는 {{site.data.keyword.Bluemix_notm}} 로컬 및 {{site.data.keyword.Bluemix_notm}} 데디케이티드에 대해 생성되는
보안 보고서 목록을 보여줍니다.
...
{{site.data.keyword.Bluemix_notm}} 카탈로그에 표시하려는 서비스가 있으면 서비스 브로커를 구현하고 등록해야 합니다. 브로커를 등록한 후, 로컬 또는 데디케이티드 인스턴스에서 해당 서비스에 액세스할 수 있는 조직을 선택할 수 있습니다.
...
<p><strong>참고</strong>: 로컬 또는 데디케이티드 환경의 서비스 브로커를 작성하는 경우, 서비스 정의 JSON 파일의 "tags" 필드에 `customer_dedicated`를 지정해야 합니다.</p>
</li>
...
사용자를 단독으로 또는 그룹으로 추가하고 사용자 권한을 볼 수 있습니다. 공통적으로, 사용자가 LDAP(Lightweight Directory Access Protocol)을 통해 회사의 사용자 레지스트리에서 {{site.data.keyword.Bluemix_notm}} 인스턴스에 추가됩니다. **관리** 권한이 지정되면 기타 사용자에 대해서도 권한을 설정하고 관리할 수 있습니다. **관리 &gt; 사용자 관리**를 클릭하십시오. 

사용자 관리 페이지에 로컬 또는 데디케이티드 인스턴스의 모든 사용자가 표시됩니다. 각 사용자의 권한이 테이블에서 아이콘을 통해 표시됩니다. 권한은 다음과 같을 수 있습니다. 없음, **관리**, **로그인**, **카탈로그**, **보고서** 및 **사용자**
**관리** 및 **로그인**` 권한은 **설정(On)** 또는 **해제(Off)**로 설정될 수 있으며, 남아 있는 권한은 아이콘으로 나타내는 대로 해당 권한에 대해 **읽기** 또는 **쓰기** 액세스 권한을 포함하여 특정 액세스 유형으로 사용 또는 사용 안함으로 설정됩니다. 각 유형 및 아이콘에 대한 설명은 [권한](#permissions)의 내용을 참조하십시오. 

### 사용자에 대한 작업

사용자의 권한에 대한 **읽기** 또는 **쓰기** 액세스 권한에 따라, 기존 사용자를 검색하고 사용자를 제거하며 사용자를 개별적으로나 그룹으로 추가할 수 있습니다. 참고로, 사용자에게 **관리** 권한이 있는 경우 환경에서 사용자 관리를 위해 태스크를 완료할 전체 액세스 권한이 있습니다. 다음 태스크를 완료하여 사용자를 관리할 수 있습니다. 다음 목록을 검토하여 사용자의 액세스 레벨을 통해 사용자가 특정 사용자 관리 태스크를 완료할 수 있도록 하는지 확인하십시오.

* 사용자 찾기. 사용자 권한에 대해
**읽기** 또는 **쓰기** 액세스 권한으로 **검색** 필드를 사용하여 테이블에서 사용자를 찾을 수 있습니다.

* 단일 사용자를 추가합니다. **쓰기** 액세스 권한과 함께 **관리** 권한 또는
...
* LDAP 디렉토리에서 사용자 그룹을 추가합니다. **쓰기** 액세스 권한과 함께 **관리** 권한 또는
**사용자** 권한이 있으면 사용자를 추가할 수 있습니다.
...
* 사용자 ID, 사용자 이메일 주소 및 사용자를 추가할 조직이 포함되어 있는 스프레드시트를 가져와서 사용자 그룹을 추가합니다. **쓰기** 액세스 권한과 함께 **관리** 권한 또는 **사용자** 권한이 있으면 사용자를 추가할 수 있습니다.
...
* 사용자 제거. **쓰기** 액세스 권한과 함께 **관리** 권한 또는 **사용자** 권한이 있으면 영구적으로 환경에서 사용자를 제거할 수 있습니다.

    1. 사용자를 찾고 ![삭제](images/icon_trash.svg) 아이콘을 클릭하십시오.
    2. **제거**를 클릭하십시오.

* 사용자가 속한 조직 및 권한을 편집하려면 **관리** 권한이 있어야 합니다. 사용자의 권한을 편집하려면 사용자를 찾고 사용자 이름을 클릭하십시오. **사용자 편집** 페이지에서 권한을 사용 또는 사용 안함으로 설정할 수 있습니다.

    * 목록에서 **설정**을 선택하여 **관리** 또는 **로그인** 권한을 사용으로 설정하십시오.
    ...
    **참고**: **관리** 권한을 **설정**으로 설정하면 **쓰기** 액세스 권한이 포함된 기타 모든 권한이 설정됩니다.

* 특정 조직에서 사용자를 추가하거나 제거하려면 **쓰기** 액세스 권한과 함께 **관리** 권한 또는 **사용자** 권한이 있어야 합니다.
...
사용자는 특정 태스크를 완료할 수 있도록 하는 특정 액세스 레벨로 다음 권한이 지정될 수 있습니다.

*표 7. 권한*

| **사용자 권한** | **설명** |       
|-----------------|-------------------|
| 관리 | **관리** 권한이 **설정**으로 설정된 사용자는 기타 사용자에 대해 권한을 편집하도록 허용됩니다. 권한이 설정되어 있으면 기타 모든 권한에 대한 전체 액세스 권한을 자동으로 사용 가능하게 합니다. 이 테이블에서 각 권한마다 아웃라인된 태스크 이외에, 관리자는 이벤트 구독을 설정하여 유지보수나 인시던트에 대해 직접 경보를 받고 유지보수를 스케줄하며 콘솔 컴포넌트의 검증 확인을 실행하고 환경의 공간 및 조직을 작성할 수도 있습니다. |
| 로그인 | **로그인** 권한이 **설정**으로 설정된 사용자는 {{site.data.keyword.Bluemix_notm}} 사용자 인터페이스에서 관리 페이지 옵션을 보도록 허용됩니다. 권한이 사용으로 설정된 사용자는 [시스템 정보](#oc_system) 및 [리소스 사용량](#oc_resource) 타일에 액세스할 수 있습니다. 이 권한이 없으면 사용자가 관리 메뉴 옵션을 보거나 이에 액세스할 수 없습니다. |
...
| 보고서 | **보고서** 권한이 있는 사용자는 보안 보고서에 대해 **읽기** 또는 **쓰기**(수정)에 대한 액세스 권한이 지정될 수 있습니다. 읽기 액세스 권한을 통해 사용자는 보고서 및 로그 타일에 액세스하여 보고서를 다운로드할 수 있습니다. 쓰기 액세스 권한을 통해 사용자는 [보고서 및 로그](#oc_report) 타일을 보고 CLI를 사용하여 새 보고서를 업로드하고 사용자가 액세스할 새 카테고리를 작성할 수 있습니다. |
...
권한은 다음 아이콘으로 나타내는 대로 해당 권한에 대해 **읽기** 또는
**쓰기** 액세스 권한이 있는 사용자에 대해 사용으로 설정될 수 있습니다.

* 권한과 연관된 ![사용, 선택 표시로 표시됨](images/icon_enabled.svg) 아이콘은 이 권한이 사용으로 설정되었음을 의미합니다.
* ![읽기, 눈 모양으로 표시됨](images/icon_read.svg) 아이콘은 사용자가 해당 권한에 대해 **읽기**(읽기 전용) 액세스 권한을 갖고 있음을 의미합니다.
* ![쓰기, 연필로 표시됨](images/icon_write.svg) 아이콘은 사용자가 해당 권한에 대해 **쓰기**(편집, 추가 또는 제거) 액세스 권한을 갖고 있음을 의미합니다.
...
관리 콘솔에 로그인해야 합니다. `admin` 권한이나 `write` 액세스가 포함된
`users` 권한이 있으면 사용자를 추가하거나 제거할 수 있습니다. 다른 사용자의 권한을 편집하려면
`admin` 권한이 있어야 합니다. 
...
조직을 나열하려면 `read` 액세스가 있는 `users` 권한이
...
환경에 추가되었는지 판별할 수 있습니다. 등록된 사용자를 나열하려면 `read` 액세스가 있는 `users` 권한이
