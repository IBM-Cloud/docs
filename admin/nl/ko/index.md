...
{{site.data.keyword.Bluemix_notm}} 상태 페이지를 사용하여 {{site.data.keyword.Bluemix_notm}} 인스턴스의 상태를 모니터할 수 있습니다. **{{site.data.keyword.avatar}}** 아이콘 ![아바타](../support/images/account_support.svg)를 클릭한 후 **상태**를 선택하십시오. 
...
사용자를 단독으로 또는 그룹으로 추가하고 사용자 권한을 볼 수 있습니다. 공통적으로, 사용자가 LDAP(Lightweight Directory Access Protocol)을 통해 회사의 사용자 레지스트리에서 {{site.data.keyword.Bluemix_notm}} 인스턴스에 추가됩니다. **수퍼유저** 권한이 지정된 경우에는 다은 사용자의 권한을 설정하고 관리할 수도 있습니다. **관리 &gt; 사용자 관리**를 클릭하십시오. 

사용자 관리 페이지에 로컬 또는 데디케이티드 인스턴스의 모든 사용자가 표시됩니다. 각 사용자의 권한이 테이블에서 아이콘을 통해 표시됩니다. 권한은 없음, **수퍼유저**, **기본 액세스**, **카탈로그**, **보고서**, **사용자**입니다.
**수퍼유저** 권한과 **기본 액세스** 권한을 **설정** 또는 **해제**로 설정할 수 있으며 나머지 권한은 아이콘으로 표시되는 대로 특정 액세스 유형(해당 권한의 **읽기** 또는 **쓰기** 액세스 권한 포함)으로 사용 또는 사용 안함으로 설정됩니다. 각 유형 및 아이콘에 대한 설명은 [권한](#permissions)의 내용을 참조하십시오. 

### 사용자에 대한 작업

사용자의 권한에 대한 **읽기** 또는 **쓰기** 액세스 권한에 따라, 기존 사용자를 검색하고 사용자를 제거하며 사용자를 개별적으로나 그룹으로 추가할 수 있습니다. **수퍼유저** 권한이 있는 경우에는 환경에서 사용자 관리를 위해 태스크를 완료할 수 있는 전체 액세스 권한을 갖습니다. 다음 사용자 관리 태스크와 각 태스크를 완료해야 하는 액세스 레벨을 검토하십시오. 

* 사용자 찾기. **읽기** 또는 **쓰기** 액세스 권한이 있고 사용자 이름 전체 또는 일부를 알고 있는 경우 **검색** 필드를 사용하여 테이블에서 사용자를 찾을 수 있습니다. 
...
| **사용자 권한** | **설명** |       
|-----------------|-------------------|
| 수퍼유저 | **수퍼유저** 권한이 **설정**으로 설정된 사용자는 다른 사용자의 권한을 편집할 수 있습니다. 권한이 설정되어 있으면 기타 모든 권한에 대한 전체 액세스 권한을 자동으로 사용 가능하게 합니다. 이 테이블에서 각 권한에 대해 간략히 설명된 태스크 이외에 이벤트 구독을 설정하여 유지보수 또는 인시던트에 대해 직접 경보를 받고 유지보수를 스케줄링하며 콘솔 컴포넌트에 대한 확인 검사를 실행하고 환경의 영역과 조직을 작성할 수도 있습니다.  |
| 기본 액세스 | **기본 액세스** 권한이 **설정**으로 설정된 사용자는 {{site.data.keyword.Bluemix_notm}} 사용자 인터페이스에서 관리 페이지 옵션을 볼 수 있습니다. 권한이 사용으로 설정된 사용자는 [시스템 정보](#oc_system) 및 [리소스 사용량](#oc_resource) 타일에 액세스할 수 있습니다. 이 권한이 없으면 사용자가 관리 메뉴 옵션을 보거나 이에 액세스할 수 없습니다. |
