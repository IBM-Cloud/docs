...
# {{site.data.keyword.Bluemix_notm}} 환경 설정의 우수 사례
{: #patterns}
...
{:caption="표 1. 핵심 요소에 대한 설명" caption-side="top"}
...
영역은 사용자가 애플리케이션 및 서비스를 개발하고 실행할 수 있는 조직에서 예약된 영역입니다. 조직에서 임의 수의 영역을 작성할 수 있습니다.
영역에 대한 액세스 권한을 가진 사용자를 제어할 수 있습니다. 자세한 정보는 [영역](/docs/admin/orgs_spaces.html#spaceinfo "영역")을 참조하십시오.
...
* {{site.data.keyword.Bluemix_notm}} 퍼블릭에서 조직 할당량을 보고 관리하는 방법에 대한 자세한 정보는 [할당량 관리](/docs/admin/orgs_spaces.html#managequota "할당량 관리")를 참조하십시오.
...
* 조직의 구성원에게 [사용자 역할](/docs/admin/users_roles.html#userrolesinfo "사용자 역할")을 부여할 수 있습니다. 이러한 역할은 조직 내 액세스 레벨을 정의하고 영역 및 해당 리소스에 액세스할 수 있는 사용자를 제한합니다. 예를 들어, 서로 다른 영역에 대한 서로 다른 권한을 사용자에게 부여할 수 있습니다.
...
[관리 역할](/docs/admin/index.html#oc_useradmin "관리 역할")은 {{site.data.keyword.Bluemix_notm}} 데디케이티드 또는 {{site.data.keyword.Bluemix_notm}} 로컬 계정을 관리하도록 사용자에게 부여할 수 있는 권한을 정의합니다.
...
3. 계정 관리에 필요한 권한과 일치하도록 각 사용자에 대한 하나 이상의 역할을 선택하십시오.
   ...
   * 관리자 역할: 계정에서 둘 이상의 사용자에게 이 역할을 부여하십시오. 이 역할의 사용자에게 전체 조직을 관리할 권한이 있습니다.
   * 사용자 역할: 읽기 또는 쓰기 권한으로 이 역할을 구성할 수 있습니다. 조직 관리자가 사용자를 계정 및 해당 조직에 추가할 수 있도록 이 조직 관리자에게 쓰기 권한을 포함한 해당 역할을 부여하십시오. 계정의 구성원 목록을 볼 수 있는 액세스 권한이 필요한 조직 관리자에게 읽기 권한을 포함한 해당 역할을 부여하십시오.
   * 카탈로그 역할: 읽기 또는 쓰기 권한으로 이 역할을 구성할 수 있습니다. 사용자 세트에 쓰기 권한이 포함된 이 역할을 부여하여 이들이 {{site.data.keyword.Bluemix_notm}} 카탈로그에서 사용자에게 표시될 Bluemix 서비스와 스타터를 정의하고 관리하도록 허용하십시오. 조직 관리자에게 읽기 권한을 포함한 해당 역할을 부여하십시오.
   * 보고서 역할: 읽기 또는 쓰기 권한으로 이 역할을 구성할 수 있습니다. 사용자 세트에 쓰기 권한이 포함된 이 역할을 부여하여 이들이 읽기 권한이 있는 다른 사용자가 다운로드할 수 있는 보고서를 보고 작성하도록 허용하십시오. 관리 팀의 모든 구성원에게 읽기 권한을 부여하십시오.
