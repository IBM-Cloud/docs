...
* **情境 3：依專案隔離**
  ...
  說明：您的公司執行黑客松 (Hackathon) 來識別新服務。  

  解決方案：您可以根據每個黑客松 (Hackathon) 各定義一個組織，並使用該組織來進行沙盤推演。在執行黑客松 (Hackathon) 之後，您可以將沙盤推演組織提升到您帳戶中的其他組織。
...
* 如果您需要為不同的使用者群組強制執行界限。例如，開發人員無法開發應用程式並進行測試。您需要一組不同的使用者來測試應用程式。在此情境中，您可以建立兩個空間，一個供應用程式的開發人員使用，一個供應用程式的測試人員使用。然後，授與各組使用者正確空間的存取權。
