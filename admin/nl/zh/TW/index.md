...
| 我能執行哪些操作？ | 詳細資料 |    
|----------------|---------|
|監視系統用量 | 按一下**管理 &gt; 用量**。檢視系統資訊、監視 CPU 使用率，以及計劃用量，以便做出最佳業務決策。請參閱[檢視使用資訊](index.html#oc_resource)。|
...
|擴充通知以及設定事件訂閱 | 按一下**管理 &gt; 系統資訊 &gt; *數字* 個擱置的更新**。您可以使用 Webhook 與您選擇的 Web 服務整合，以設定更新或事件的事件通知訂閱。請參閱[通知及事件訂閱](index.html#oc_eventsubscription)。 |
...
| 重要事件 | 您可以在「狀態」頁面上收到有關重要事件的警示。按一下**帳戶和支援**圖示 ![帳戶和支援](../support/images/account_support.svg)，然後選取**狀態**。若要擴充通知功能，您可以設定事件訂閱來傳送電子郵件給您選擇的收件者。或者，您可以將訂閱設定為使用 Webhook，將「管理」頁面的通知與您選擇的 Web 服務整合。  |  
...
<ol>
<li>按一下<strong>帳戶和支援</strong>圖示 ![帳戶和支援](../support/images/account_support.svg) &gt; <strong>帳戶</strong> &gt; <strong>用量詳細資料</strong>。</li>
<li>選取您要查看資料的組織。</li>
<li>您可以查看下列種類的用量詳細資料：
<ul>
<li>含用量的運行環境應用程式</li>
...
您可以新增單一使用者或使用者群組，並且檢視使用者許可權。通常會透過「輕量型目錄存取通訊協定 (LDAP)」，從您公司的使用者登錄，將使用者新增至 {{site.data.keyword.Bluemix_notm}} 實例。如果您已獲指派 **Admin** 許可權，您也可以設定及管理其他使用者的許可權。按一下**管理 &gt; 使用者管理**。

「使用者管理」頁面會顯示本端或專用實例的所有使用者。每一位使用者的許可權會以圖示顯示在表格中。許可權可以是下列各項：None、**Admin**、**Login**、**Catalog**、**Reports** 和 **Users**。**Admin** 和 **Login** 許可權可以設為**開啟**或**關閉**，而其餘的許可權則是以特定存取類型來啟用或停用，包括該許可權的 **Read** 或 **Write** 存取權，以圖示表示。如需每一種類型的說明以及圖示的說明，請參閱[許可權](#permissions)。

### 處理使用者

依據您對使用者許可權的 **Read** 或 **Write** 存取權，您可以個別或依群組搜尋現有使用者、移除使用者，以及新增使用者。請注意，如果您有 **Admin** 許可權，則表示您有完整存取權可以在該環境中完成管理使用者的任何作業。您可以完成下列作業來管理使用者。請檢閱下列清單，以查看您的存取層次是否可讓您完成特定的使用者管理作業：

* 尋找使用者。如果您有使用者許可權的 **Read** 或 **Write** 存取權，則可以使用**搜尋**欄位在表格中尋找使用者。

* 新增單一使用者。如果您有 **Admin** 許可權，或是具有 **Write** 存取權的 **Users** 許可權，則可以新增使用者。
...
* 從 LDAP 目錄新增一組使用者。如果您有 **Admin** 許可權，或是具有 **Write** 存取權的 **Users** 許可權，則可以新增使用者。
...
* 匯入試算表，來新增一組使用者，該試算表包括使用者 ID、使用者電子郵件位址，以及您計劃要新增使用者的組織。如果您有 **Admin** 許可權，或是具有 **Write** 存取權的 **Users** 許可權，則可以新增使用者。
...
**附註**：輸入符合使用者登錄中所使用值的使用者 ID。 
  ...
  **附註**：只要您有所有必要直欄，試算表內的直欄可以是任意順序。如果匯入成功，您會收到確認訊息，指出已新增所有使用者。如果部分使用者的匯入成功，但其他使用者不然，請檢閱錯誤訊息，以對無法新增的使用者採取動作。

* 移除使用者。如果您有 **Admin** 許可權，或是具有 **Write** 存取權的 **Users** 許可權，則可以將使用者從該環境中永久移除。

    1. 找出使用者，然後按一下 ![刪除](images/icon_trash.svg) 圖示。
    2. 按一下**移除**。

* 若要編輯使用者所屬的許可權和組織，您必須要有 **Admin** 許可權。若要編輯使用者的許可權，請找出使用者，然後按一下使用者名稱。從**編輯使用者**頁面中，您可以啟用或停用許可權：

    * 從清單中選取**開啟**，以啟用 **Admin** 或 **Login** 許可權。
    * 從清單中選取 **Read**，讓使用者擁有該許可權的 **Read**（唯讀）存取權，或選取 **Write**，讓使用者擁有該許可權的 **Write**（編輯、新增及移除）存取權。
    * 選取**關閉**以停用任何許可權。
    
    **附註**：若將 **Admin** 許可權設為**開啟**，會將所有其他許可權設為具有 **Write** 存取權。

* 若要在特定組織中新增或移除使用者，您必須要有 **Admin** 許可權，或是具有 **Write** 存取權的 **Users** 許可權。
...
使用者可獲指派下列下列許可權，這些許可權具有可讓使用者完成特定作業的特定存取層次：

*表 7. 許可權*

| **使用者許可權** | **說明** |       
|-----------------|-------------------|
| Admin | **Admin** 許可權設為**開啟**的使用者，可以編輯其他使用者的許可權。如果這個許可權設為開啟，即會自動啟用對所有其他許可權的完整存取權。除了此表格中針對各項許可權所描述的作業之外，Admin 還可以設定事件訂閱來直接取得維護作業或事件的警示、排定維護作業、對主控台元件執行驗證檢查，以及為該環境建立組織和空間。 |
| Login | **Login** 許可權設為**開啟**的使用者，可以看到 {{site.data.keyword.Bluemix_notm}} 使用者介面中的「管理」頁面選項。啟用該許可權的使用者，可以存取[系統資訊](#oc_system)和[資源用量](#oc_resource)磚。如果沒有此許可權，使用者就無法查看或存取「管理」功能表選項。 |
