...
* 通过导入包含用户标识、用户电子邮件地址和计划向其添加用户的组织来添加用户组。
  ...
  3. 为必需列填写用户值。如果使用的不是 LDAP 目录，请将必需列和可选列标题用于用户导入。
  4. 保存文件，然后单击**上传文件**。
...
| 目录 | 可以为拥有 `catalog` 许可权的用户分配针对本地或专用实例中哪些服务可用执行 `view`（查看）或 `write`（写入，即修改）操作的访问权。 |  
| 登录 | 拥有 `login` 许可权的用户可以查看“管理”页面。没有此许可权，用户无法查看“管理”菜单选项。 |
...
您可以使用 `Admin` REST API 从 {{site.data.keyword.Bluemix_notm}} 实例中除去用户。您必须拥有具有 `write` 访问权的 `users` 许可权，才能除去用户。要除去用户，必须提供用户的用户标识。运行以下命令：`curl -v -b ./cookies.txt -X DELETE https://<your_host>.ibm.com/codi/v1/users?user_id=<some_user_id@domain.com>`
{: codeblock}
