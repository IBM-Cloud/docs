...
1. 시작하려면 앱의 개요 페이지에서 **GIT 추가**를 클릭하십시오.  
2. 열린 창에서 **스타터 애플리케이션 패키지로 저장소 채우기 및 Delivery Pipeline 사용(빌드 및 배치)** 선택란이 선택되어 있는지 확인하십시오. Git 저장소가 작성됩니다. 스타터 코드가 사용 가능하면 저장소에 로드됩니다. 또한 {{site.data.keyword.jazzhub}}에서 실행 중인 Delivery Pipeline 서비스에 의해 애플리케이션이 배치됩니다.  
