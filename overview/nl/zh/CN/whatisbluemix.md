...
# 什么是 {{site.data.keyword.Bluemix_notm}}？
{: #bluemixoverview}
...
出于等待时间或安全考虑，可以将应用程序部署到不同的 {{site.data.keyword.Bluemix_notm}} 区域。您可以选择部署到一个区域或跨多个区域部署。有关更多信息，请参阅[区域](whatisbluemix.html#ov_intro_reg)。
...
## {{site.data.keyword.Bluemix_notm}} Cloud Foundry 的工作方式
{: #howwork}
...
## {{site.data.keyword.Bluemix_notm}} 先决条件
{: #prereqs}
