...
|Opcional | [{{site.data.keyword.cloudant}}](/docs/services/Cloudant/index.html#Cloudant) | O {{site.data.keyword.cloudant}} fornece acesso a uma camada de dados do JSON NoSQL totalmente gerenciada que sempre está ativa. Esse serviço é compatível com o CouchDB e acessível por meio de uma interface HTTP simples de usar para modelos de aplicativos móveis e da web. Para obter mais informações, veja a [documentação ![Ícone de link externo](../icons/launch-glyph.svg)](http://docs.cloudant.com/BluemixLocal.html){: new_window} completa e os [requisitos de hardware ![Ícone de link externo](../icons/launch-glyph.svg)](http://docs.cloudant.com/BluemixLocalHardware.html){: new_window} para um ambiente local. |
...
**Nota**: se você optar por hospedar o {{site.data.keyword.Bluemix_local_notm}} na opção de hardware {{site.data.keyword.Bluemix_notm}} Local System, o processo de configuração poderá ser diferente, em que você não precisará fornecer tantas informações para o representante IBM. Além disso, suas funções e responsabilidades durante as fases de concepção e progressão também podem ter o escopo reduzido em razão do modelo de manutenção do dispositivo PureApplication, ao contrário do modelo de gerenciamento que é necessário para usar o VMware de propriedade do cliente.
