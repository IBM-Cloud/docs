...
{: tsResolve}
  ...
  1. {{site.data.keyword.Bluemix_notm}} 대시보드로 이동하여 대시보드 오른쪽 상단의 **계정 및 지원** 아이콘 ![계정 및 지원](images/account_support.png)을 클릭한 다음 **조직 관리**를 선택하십시오.
...
계정을 삭제한 다음 조직의 구성원으로 참여할 수 있도록 초대를 받아야 합니다. 이전 계정을 삭제하고 구성원으로 계정에 참여하려면 다음 단계를 수행하십시오. 
  ...
  1. [ID 지원 부서](mailto:id@bluemix.net)에 연락하여
지원 티켓을 열고 계정 삭제를 요청하십시오. 이전 계정과 연관된 데이터가 있어
...
`BXNUI0001E: 세션이 있는지 판별할 수 없습니다. 브라우저를 다시 로드하십시오.`


`BXNUI0016E: 애플리케이션 및 서비스를 검색할 수 없습니다.`
...
## {{site.data.keyword.Bluemix_notm}} 대시보드에서 앱 또는 서비스를 로드할 수 없음
{: #ts_dashboard}

조직이나 영역에서 대시보드에 액세스할 수 없는 경우 {{site.data.keyword.Bluemix_notm}} 대시보드에 앱 또는 서비스 정보가 표시되지 않을 수 있습니다. 
 

{{site.data.keyword.Bluemix_notm}} 사용자 인터페이스에 로그인하면 이전에 앱을 작성하고 일부 서비스를 사용한 적이 있더라도 대시보드에서 사용할 수 있는 항목이 없습니다.
{: tsSymptoms}
 

이 문제점은 서비스 인스턴스를 앱에 바인드했지만, 조직 또는 영역에 대한 서비스 권한이 변경되어 서비스에 더 이상 액세스할 수 없는 경우에 발생할 수 있습니다. 
{: tsCauses}
  	

이 문제점을 해결하려면
cf 명령행 인터페이스에 `cf services`를 입력하여 서비스 인스턴스를 나열하고
서비스를 식별하십시오. 그런 다음 `cf delete-service` 명령을 사용하여
더 이상 액세스할 수 없는 서비스 인스턴스를 삭제하십시오.
{: tsResolve}
...
`BXNUI2032E:
자원을 작성하기 위해 Cloud Foundry로 접속하는 중에 오류가 발생했습니다. 자원:
service_instances. Cloud Foundry 메시지: "조직의 서비스 한계를 초과했습니다."`
...
`BXNUI0096E: 애플리케이션을
작성할 수 없습니다. 계정이 취소되었거나 일시중단되었으므로 비활성 상태입니다.`
