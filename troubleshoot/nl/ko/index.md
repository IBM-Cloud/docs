...
{: tsResolve}
  ...
  1. {{site.data.keyword.Bluemix_notm}} 대시보드로 이동하고 메뉴 표시줄에서 **계정 및 지원** 아이콘 ![계정 및 지원](images/account_support.svg)을 클릭한 후 **조직 관리**를 선택하십시오.
...
{: tsResolve}
  ...
  * 계정 및 지원 ![계정 및 지원](images/account_support.svg)에서, 영역을 작성할 조직을 선택한 후 **영역 작성**을 클릭하십시오.
  * cf 명령행 인터페이스에서 `cf create-space <space_name> -o <organization_name>`을 입력하십시오.

다시 시도하십시오. 이 메시지가 다시 표시되면 [Bluemix 상태](https://status.eu-gb.bluemix.net/){: new_window} 페이지로 이동하여 서비스 또는 컴포넌트에 문제가 있는지 여부를 확인하십시오.
...
## 2바이트 문자는 Liberty 애플리케이션이 {{site.data.keyword.Bluemix_notm}}에 푸시될 때 적절히 표시되지 않습니다.
{: #ts_doublebytes}
...
{: tsResolve} 
  ...
  * 다음 방법 중 하나로 시작 명령을 지정하십시오. 
      * cf 명령행 인터페이스를 사용하십시오. 예: 
		...
		```
	  * [package.json](https://docs.npmjs.com/json){: new_window} 파일을 사용하십시오. 예:
	    ```
		{
      ...
  	   "scripts": {
