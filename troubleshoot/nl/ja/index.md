...
`BXNUI0016E: アプリケーションとサービスを取得できませんでした。`
...
## {{site.data.keyword.Bluemix_notm}} ダッシュボードがアプリまたはサービスをロードできない
{: #ts_dashboard}
...
`BXNUI0096E: アプリケーションを作成できませんでした。アカウントは取り消されたか中断されたため、非アクティブです。`
...
`BXNUI0097E: 新規アプリケーションを追加できませんでした。現在の組織および地域に関連付けられたスペースはありません。ダッシュボードで、「スペースの作成」をクリックします。スペースが作成されたら、やり直してください。`
...
`NOTICE` メッセージは情報を提供するメッセージであり、必ずしも問題の発生を示すものではありません。
これらのメッセージのロギングは、ビルドパックの nginx-defaults.conf ファイルに含まれるロギング・レベルを stderr notice から stderr error に変更することで停止できます。例:
{: tsResolve}
