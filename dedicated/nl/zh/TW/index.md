...
|IBM 專用 {{site.data.keyword.mql}} PR 30 容量增加 | 此環境允許部署及執行 {{site.data.keyword.mql}} 實例，最大總容量為 30 個服務實例。 |
|專用 {{site.data.keyword.mobilepushshort}} 服務 12 GB 容量增加 | 此環境允許在專用環境內部署及執行 {{site.data.keyword.Bluemix_notm}} {{site.data.keyword.mobilepushshort}} 服務（包含 Liberty 及 Node.js 執行時期）。{{site.data.keyword.mobilepushshort}} 服務對於 {{site.data.keyword.cloudant}} 的必要相依關係是設計容量至少為 500 GB。 |
...
聯合型錄提供建立包含公用及專用服務之混合式應用程式的功能。您可以選擇根據資料隱私及安全準則來決定符合商業需求的公用服務。如果有適用於您專用環境的服務專用實例，您會看到型錄中的服務磚附加特定圖示，例如 ![「專用服務」圖示](images/dedicatedicon.png "「專用服務」圖示")。
...
您的客戶代表會與您的專責代言人及其他合作的 IBM 專家一起合作，確保您隨時擁有所需的支援。您的「專責代言人」是 {{site.data.keyword.Bluemix_notm}} 開發支援團隊內完成下列作業的單一聯絡點：
...
|執行維護 | 與 IBM 一起排定 21 天時間範圍內的必要維護。您可以提供 30 天時間範圍內無法進行的日期，而 IBM 會據此排定維護。 | IBM 及客戶皆負有責任 |
...
當 IBM 認為適用於「{{site.data.keyword.Bluemix_notm}} 專用」平台、執行時期及服務時，IBM 即會維護並安裝更新及修正程式。
...
<dt>**{{site.data.keyword.Bluemix_notm}} 的標準維護**</dt>
<dd>維護更新是根據 21 天的時間範圍內，您與 IBM 之間的協調來套用。您可以將 21 天的時間範圍內，可能不適合的特定日期或時間提供給 IBM。IBM 在排定更新時會嘗試避開那些時間。根據要求，IBM 會與您溝通排定的維護時間範圍。每月變更時間範圍預期不會影響執行中的「Bluemix 專用」環境。<p>下列影像顯示這個程序：接收擱置更新的通知、設定不適合您的日期，然後接收排定日期的通知：</p>
<p><img src="../local/images/maintenance_dates.png" alt="無法進行維護更新之日期的設定程序"></p>
<br />
**附註**：如果您不需要設定任何無法進行更新的日期，則可以核准此更新。IBM 之後會通知您要套用維護的排定日期。<br />
<br />
移至**管理 > 系統資訊**，以檢視擱置更新、設定無法進行的日期，以及核准更新。如需通知及排定擱置更新的相關資訊，請參閱<a href="../admin/index.html#oc_system">檢視系統資訊</a>。</dd>
</dl>

**重要事項**：IBM 保留視需要岔斷服務以套用緊急維護的權利。IBM 可能會變更排定的維護時間，但是一旦有這類變更以及任何緊急維護資訊時，就會通知您。

若要設定專用實例的維護，請與 IBM 指定的客戶業務代表一起找出進行標準維護的協議時間範圍。如果在維護更新作業之後發生報告過的問題，請與 IBM 業務代表協議容許 IBM 回復更新是否最符合您的權益。達成協議之後，IBM 會回復更新，以將環境還原至前一個狀態。
...
「{{site.data.keyword.Bluemix_short}} 公用」提供持續可用的平台以進行創新。有多種失敗安全的措施可以確保您的組織、空間及應用程式隨時可用。將應用程式部署至多個地理區域會啟用持續可用性，以免於非計劃性地同時損失多個硬體或軟體元件，或損失整個資料中心，以便即使在某個地理位置發生自然災難時，仍將可以使用替代地理位置中的分散式「{{site.data.keyword.Bluemix_notm}} 公用」應用程式實例。
{: shortdesc}
