...
<strong>예제</strong>:API 엔드포인트를 api.ng.bluemix.net으로 설정합니다.
...
한 번에 `apps`, `containers`, `container-groups` 또는
`vm-groups` 중 하나만 지정이 가능합니다. 지정한 항목이 없으면 cf 앱, 컨테이너, 컨테이너 그룹 및 VM 그룹이 모두 나열됩니다.
...
<strong>명령 옵션</strong>:
   ...
   <dd>수신된 모든 신호를 프로세스에 프록시합니다. 기본값은
<i>true</i>입니다. </dd>
   <dt>CONTAINER(필수)</dt>
...
<strong>명령 옵션</strong>:
   <dl>
   <dt>SOURCE_IMAGE(필수)</dt>
   <dd>소스 저장소 및 이미지 이름입니다. </dd>
   <dt>DESTINATION_IMAGE(필수)</dt>
...
|  환경 변수                              |     설명                            |
| :----------------------------- | :------------------------------ |
| CCS_BIND_APP=*&lt;appname&gt;*       | 컨테이너에 서비스를 바인드합니다. `CCS_BIND_APP` 환경 변수를 사용하여 앱을 컨테이너에 바인드하십시오. 앱은 대상 서비스에 바인드되어 브릿지 역할을 하며, 이 브릿지를 통해 {{site.data.keyword.Bluemix_notm}}는 사용자 브릿지 앱의 `VCAP_SERVICES` 정보를 실행 중인 컨테이너 인스턴스로 가져올 수 있습니다. 브릿지 앱 작성에 대한 자세한 정보는 [컨테이너에 서비스 바인딩](http://www.ng.bluemix.net/docs/containers/container_creating_ov.html#container_binding_ov){: new_window}을 참조하십시오. |
...
<ul>
   <li>VOLUME: 볼륨 ID 또는 이름입니다. </li>
   <li>CONTAINER_PATH: 컨테이너의 볼륨에 대한 절대 경로입니다. </li>
...
<strong>명령 옵션</strong>:
   ...
   <dl>
   <dt>NAME(필수)</dt>
   <dd>아직 설정되지 않은 경우, 조직의 저장소 네임스페이스를 설정하는 일회성 기능입니다. 네임스페이스를 설정한 후에는 계속하기 전에 <i>bluemix ic init</i> 명령을 사용하여 IBM Containers를 다시 초기화하십시오.</dd>
   </dl>
...
<strong>명령 옵션</strong>:
   ...
   <dl>
   <dt>FILE_SYSTEM_NAME(필수)</dt>
   <dd>파일 시스템 이름입니다. 이름에는 소문자, 숫자, 밑줄 "_" 및 하이픈 "-"이 포함될 수 있습니다. </dd>
   </dl>
